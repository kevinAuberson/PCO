...
private:
    ...
    int nextId = 0; // Id donnée au resulat quand ils arrivent
    int actualId = 0; // Id resultat actual

    std::vector<std::queue<Request>> buffer;

    std::vector<std::vector<int>> computationResult;

    std::vector<Condition> full, // Prod
                            empty; // Cons

    std::vector<int> abortedId;

    std::vector<Result> resultBuffer;
    Condition resultBufferEmpty;
