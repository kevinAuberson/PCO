...
protected:
    ...
    // Gestion de l'accès au salon et aux chaises
    unsigned MAX_WAITING;
    unsigned MAX_SALON;
    unsigned nextChair;

    // Utilisé dans la liste d'attente pour gérer l'order de passage
    std::queue<unsigned> clientInSalon;

    // Synchronisation entre thread
    PcoConditionVariable clientWait;
    PcoConditionVariable barberSleep;
    PcoConditionVariable clientWorkChair;
    PcoConditionVariable barberCutting;
};
